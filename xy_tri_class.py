...
class XY_triangular():
    def __init__(self,temperature = 3,width=10):
        ...
        for i in range(int(N/2)):
            #self.spin_config[self.spin_index[i]]=list(np.random.random(int(N/2))*2*pi)[i]#完全无序的初态
            self.spin_config[self.spin_index[i]]=list(np.zeros(int(N/2)))[i]#完全有序的初态
    ...
    def vortex_density(self):#这个函数失败了，没有能有效的实现涡旋密度的表示
    ...
    def equilibrate(self,max1_nsweeps=int(1e4),temperature=None,show = False,show_after=False,equili_yes=True):
        ...
        for k in range(max1_nsweeps):
            ...
            if ( ( abs(energy)==0 or abs(energy-energy_temp)/abs(energy)<1e-4 ) & (k>1000) ) or k == max1_nsweeps-1 :
    ...
    #            #print('energy=%.3f'%energy)
        ...
        for ki in range(int(after_steps)):
            ...
            if show_after  & (ki%100 ==0):#equilibrate中的show_after参数控制是否输出每隔1000步的态，默认false
                print('#sweeps=%i'% (ki+1))
   #             print('energy=%.3f'%energy)
                self.show()
        self.energy = np.average(dic_thermal_t['energy'])
        energy2=np.average(np.power(dic_thermal_t['energy'],2))
        self.Cv=(energy2-self.energy**2)*beta**2
        self.magnetism=np.mean(dic_thermal_t['magnetism'])
        mag2=np.average(np.power(dic_thermal_t['magnetism'],2))
        self.mag_suscept= (mag2-self.magnetism**2)/beta #np.average(dic_thermal_t['vortex_density'][n1:])
        self.totalmag=np.average(dic_thermal_t['totalmag'])
        totalmag2=np.average(np.power(dic_thermal_t['totalmag'],2))
        self.totalmag_suscept= (totalmag2-self.totalmag**2)*beta
        ...
        if equili_yes :#show_figure 用来控制要不要进行接下来的一系列绘图，默认绘制
           ...
           # def FFT (Fs,data):
           #     FL = len (data)                        # 信号长度
            #    FN = int(np.power(2, np.ceil(np.log2(FL))))    # 下一个最近二次幂
             #   FFT_y1 = np.abs(fft(data,FN))/FL*2      # N点FFT 变化,但处于信号长度
              #  Fre = np.arange(int(FN/2))*Fs/FN        # 频率坐标
               # FFT_y1 = FFT_y1[range(int(FN/2))]      # 取一半
                #return Fre, FFT_y1
    
            #Fre, FFT_y1 = FFT(Fs,dic_thermal_t['magnetism'])
            #plt.figure(figsize=(5,4), dpi=200)
            #plt.plot(Fre,FFT_y1)
            #plt.grid()
            #plt.savefig('f{}fft.png'.format(self.temperature), bbox_inches='tight')
            #plt.show()
    ...
    def annealing(self,T_init=2.5,T_final=0.1,nsteps = 20,show_equi=False):#同一个原始构型，不同温度下演化
        ...
        dic_thermal['energy']=[]
        dic_thermal['Cv']=[]
        dic_thermal['magnetism']=[]
        dic_thermal['totalmag']=[]
        dic_thermal['mag_suscept']=[]
        ...
        for T in dic_thermal['temperature']:
            ...
            dic_thermal['magnetism'] += [self.magnetism]
            dic_thermal['totalmag']+=[self.totalmag]
            dic_thermal['mag_suscept']+= [self.mag_suscept]
            dic_thermal['totalmag_suscept']+= [self.totalmag_suscept]
            
            
        #x=list(dic_thermal['temperature'])
        ...
        plt.plot(dic_thermal['temperature'][2:],dic_thermal['Cv'][2:])
        plt.scatter(dic_thermal['temperature'][2:],dic_thermal['Cv'][2:])
        plt.ylabel(r'$C_v$')
        plt.xlabel('T')
        plt.savefig('Cv_vs_T.png', bbox_inches='tight')
        plt.show()
        ...
        plt.plot(dic_thermal['temperature'],np.abs(dic_thermal['magnetism']))
        plt.scatter(dic_thermal['temperature'],np.abs(dic_thermal['magnetism']))
        ...
        #ycv_smooth = make_interp_spline(x, ycv)(x_smooth)#用于平滑函数
        #plt.plot(x_smooth, ycv_smooth)
        plt.plot(dic_thermal['temperature'],dic_thermal['mag_suscept'])
        plt.scatter(dic_thermal['temperature'],dic_thermal['mag_suscept'])
        ...
        #ycv_smooth = make_interp_spline(x, ycv)(x_smooth)#用于平滑函数
        #plt.plot(x_smooth, ycv_smooth)
        plt.plot(dic_thermal['temperature'],dic_thermal['totalmag_suscept'])
        plt.scatter(dic_thermal['temperature'],dic_thermal['totalmag_suscept'])
        ...
        plt.scatter(dic_thermal['temperature'],dic_thermal['energy'])
        plt.ylabel(r'$\langle E \rangle$')
        plt.xlabel('T')
        plt.savefig('E_vs_T.png', bbox_inches='tight')
        plt.show()
    ...
    def show(self,colored=False):#图形的绘制都是从左下角零点开始，而数据举证表示出来时第一个点在左上角，要翻转对应，无论方格子或箭头
        ...
        #print(U)
        fig1, ax1 = plt.subplots(figsize=(8,8/1.2),dpi=700)#500
        ...
        ax1.scatter(x_tri, y_tri, color='0', s=0.7)#画晶格散点图
